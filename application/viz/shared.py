...
# Zero-setup mode: run every chart/dashboard off the committed synthetic
# sample instead of a real Postgres export. Triggered by USE_SAMPLE=1 (the
# only thing Streamlit's `streamlit run` can pick up cleanly) or a
# `--sample` CLI flag (for the standalone `python viz/0N_*.py` scripts).
USE_SAMPLE = os.environ.get("USE_SAMPLE") == "1" or "--sample" in sys.argv

# Real export filename -> sample filename. The sample files live under a
# separate data/sample/ directory (committed — see .gitignore) so they can
# never be mistaken for, or accidentally overwrite, a real export.
_SAMPLE_FILENAMES = {
    "jobmarket.parquet": "jobmarket_sample.parquet",
    "job_skills.csv": "job_skills_sample.csv",
    "seen_jobs.csv": "seen_jobs_sample.csv",
    "skill_cooccurrence.csv": "skill_cooccurrence_sample.csv",
}
