...
Zero-setup mode (no database, no real numbers — illustrative only):
    python viz/story/build_story_data.py --sample
    # or: USE_SAMPLE=1 python viz/story/build_story_data.py
Every "note" field in the sample output says so explicitly; every number in
this mode is synthetic and must never be read as a real finding.
"""
...
from shared import USE_SAMPLE, classify_rising_cooling, week_start  # noqa: E402
...
JOBFORGE_SOURCE_LABEL = "matching pipeline (sample)" if USE_SAMPLE else "matching pipeline"
MARKET_SOURCE_LABEL = "market-intelligence system (sample)" if USE_SAMPLE else "market-intelligence system"
_SAMPLE_NOTE_PREFIX = "Illustrative sample data — generated by --sample mode, not a real finding. "
...
def build_skills() -> dict[str, Any]:
    ...
    if USE_SAMPLE:
        ...
        if not skills_path.exists():
            sys.exit(f"Missing {skills_path} — run scripts/generate_sample_data.py first.")
    else:
    ...
    return {
        ...
        "note": (
            _SAMPLE_NOTE_PREFIX + "Reads the committed synthetic job_skills_sample.csv."
            if USE_SAMPLE else
            "The matching pipeline's own matched_skills_json is always an empty list — this step uses the market-intelligence system's real per-job skill tags instead. See README.md."
        ),
    }


def build_scale_sample() -> dict[str, Any]:
    """Step 2, zero-setup mode — mirrors the shape of build_scale() with illustrative numbers only."""
    ...
    return {
        ...
        "total_scraped": 2000,
        "date_start": start.date().isoformat(),
        "date_end": (start + timedelta(weeks=weeks)).date().isoformat(),
        "weeks": weeks,
        "source_count": len(sources),
        "sources": sources,
        "inflated_run_history_sum": None,
        "note": _SAMPLE_NOTE_PREFIX + "Numbers match the size of the committed jobmarket_sample.parquet, not a real pipeline run.",
    }


def build_funnel_sample() -> dict[str, Any]:
    """Step 3, zero-setup mode — a plausible 2-gate funnel + tier split, not a real one."""
    scraped, scored = 2000, 450
    tiers = [
        {"tier": "bronze", "count": 270, "pct": round(100 * 270 / scored, 1)},
        {"tier": "silver", "count": 113, "pct": round(100 * 113 / scored, 1)},
        {"tier": "gold", "count": 67, "pct": round(100 * 67 / scored, 1)},
    ]
    return {
        "source": JOBFORGE_SOURCE_LABEL,
        "stages": [{"label": "Scraped", "value": scraped}, {"label": "Scored", "value": scored}],
        "tiers": tiers,
        "qualified_at_threshold": 240,
        "match_threshold": MATCH_THRESHOLD,
        "dedup_stage_populated_runs": 0,
        "prescreen_stage_populated_runs": 0,
        "total_runs": 24,
        "note": _SAMPLE_NOTE_PREFIX + "A plausible 2-gate funnel shape, not a real pipeline outcome.",
    }


def build_visa_sample() -> dict[str, Any]:
    """Step 4, zero-setup mode — a plausible sponsor-licence gap, not a real cross-check."""
    total, checked, licensed = 2000, 240, 24
    claims = 8
    pictogram_highlighted = round(PICTOGRAM_N * licensed / checked)
    return {
        "source": JOBFORGE_SOURCE_LABEL,
        "total_postings": total,
        "claims_sponsorship": claims,
        "claims_pct": round(100 * claims / total, 1),
        "employers_checked": checked,
        "employers_checked_pct": round(100 * checked / total, 1),
        "employers_licensed": licensed,
        "licensed_pct_of_checked": round(100 * licensed / checked, 1),
        "claims_and_checked_overlap": 3,
        "claims_and_licensed_overlap": 0,
        "pictogram_n": PICTOGRAM_N,
        "pictogram_highlighted": pictogram_highlighted,
        "register_source": "Illustrative — no real register was checked in --sample mode.",
        "note": _SAMPLE_NOTE_PREFIX + "No real UK Home Office register cross-check ran; these numbers only exercise the pictogram's rendering.",
    }


def build_salary_vs_score_sample(rng: np.random.Generator) -> dict[str, Any]:
    """Step 6, zero-setup mode — a synthetic scatter with a mild positive score/salary correlation."""
    n = 120
    scores = rng.uniform(40, 100, size=n)
    noise = rng.normal(0, 12000, size=n)
    salaries = np.clip(35000 + scores * 700 + noise, 22000, 145000)
    points = [
        {
            "score": round(float(s), 1),
            "salary": round(float(sal)),
            "tier": "gold" if s >= GOLD_MIN else ("silver" if s >= SILVER_MIN else "bronze"),
            "title": "Sample Role",
            "company": "Sample Employer",
            "source": "sample",
        }
        for s, sal in zip(scores, salaries)
    ]
    return {
        "source": JOBFORGE_SOURCE_LABEL,
        "points": points,
        ...
        "gold_min": GOLD_MIN,
        "silver_min": SILVER_MIN,
        "excluded_day_rate_like": 0,
        "note": _SAMPLE_NOTE_PREFIX + "Synthetic points with a mild positive score/salary correlation baked in — not a real relationship.",
    }
...
def main() -> None:
    if USE_SAMPLE:
        ...
        scale = build_scale_sample()
        funnel = build_funnel_sample()
        visa = build_visa_sample()
        salary_score = build_salary_vs_score_sample(rng)
    else:
    ...
    print("  hook              (derived from scale + visa)")
