...
<p align="center">
  <img src="../../../docs/images/story_hook.png" alt="Story hook step" width="48%">
  <img src="../../../docs/images/story_visa.png" alt="Story visa-reality pictogram step" width="48%">
</p>

*(Both generated in `--sample` mode — illustrative numbers, not real
findings. See "What each step claims" below and the root README's "Honest
limitations" section.)*
...
`data/story.json` is precomputed — the JS never aggregates anything. Never
committed (see `.gitignore`) — regenerate it before serving the page.

**Zero-setup (no database, illustrative numbers only):**

```bash
python viz/story/build_story_data.py --sample
# or: USE_SAMPLE=1 python viz/story/build_story_data.py
```

Every step's `source` label gets `" (sample)"` appended and every `note`
field says explicitly that the numbers are synthetic — see "Honest
limitations" in the root README before reading anything from a `--sample`
run as a real finding.
